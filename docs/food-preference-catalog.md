...
- 기준일: **2026-09-26**
- `FoodAllergy.entries`는 백엔드 DB와 합의한 알레르기 26개를 같은 순서로 정의한다.
- 백엔드 요청·응답과 저장에는 표의 **ID**를 기준으로 사용하고, 사용자 화면에는
  `labelRes`의 표시 이름을 사용한다.
- 알레르기를 추가·삭제하거나 ID를 변경할 때는 백엔드 DB, `FoodAllergy`, 문자열
  리소스와 이 문서를 함께 수정한다.
- `밀가루`는 별도 알레르기 항목이 아니다. 백엔드 기준 항목은 `wheat`/`밀` 하나다.
  상품 설명의 `밀가루 무첨가` 같은 일반 문구는 이 카탈로그의 항목과 별개다.
